...
-- ==========================================
-- 🛡️ HỆ THỐNG ANTI-AFK TOÀN DIỆN V2 🛡️
-- ==========================================
local VirtualInputManager = game:GetService("VirtualInputManager")
local UserInputService = game:GetService("UserInputService")

-- 1. Chặn tín hiệu theo dõi AFK lên Server
local oldNamecall;
...
-- 2. Vô hiệu hóa AFK mặc định & Hỗ trợ treo thu nhỏ (Background Tab)
pcall(function()
...
-- 3. Tự động nhảy (Chống kẹt & báo hiệu client đang hoạt động)
task.spawn(function()
...
task.spawn(function()
    ...
    while task.wait(0.25) do 
        ...
        if numB > 0 and numP > 0 then
            local bulkAssignments = {}
            local euids = vm:Get("Euids")
            
            -- phan bo vong lap giup cpu xu ly it toan hoc hon
            for i, petID in ipairs(petIDs) do
                if euids[petID] then
                    local bKey = availableBreakables[(i % numB) + 1]
                    bulkAssignments[petID] = bKey
                end
            end
...
task.spawn(function()
    -- click dao nhe nhang giam delay
        ...
        pcall(function()
            for key, info in pairs(vm:Get("AllBreakables")) do
                if info.pid == zone then
                    ...
                    count = count + 1
                    if count >= 10 then break end -- giam xuong 10 de tranh spam packet
                end
            end
        end)
    end
