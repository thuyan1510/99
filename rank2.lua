...
local function CreateSimpleDummy()
    ...
    hrp.Anchored = true
    hrp.CFrame = CFrame.new(DummyPlatformPos)
    hrp.Parent = dummy
    ...
    for _, folderName in ipairs({"Map", "Map2", "Map3", "Map4", "Map5", "Map6"}) do
        ...
        if mapFolder then
            for _, zoneFolder in pairs(mapFolder:GetChildren()) do
                ...
                if num then
                    ZoneNumberToFolder[num] = zoneFolder
                end
            end
        end
    end
...
local function GetZoneFolderByOffset(offset)
    ...
    local folder = ZoneNumberToFolder[targetNum]
    return folder, targetNum
end
...
-- ==========================================
-- Mail và Free Gifts gộp chung 15s
task.spawn(function()
...
-- Daycare, pet slots, ultimate vẫn giữ riêng vì tần suất khác
task.spawn(function()
...
local function CheckItemExact(itemName)
    local inv = GetCachedSave().Inventory.Misc; if not inv then return false, nil end
                ...
                if quest.name == "USE_FLAG" then
                    if not actionTakenThisLoop then
                        if os.clock() - (vm:Get("ActionTime_" .. quest.goalId) or 0) > 4 then 
                            ...
                            for uid, item in pairs((GetCachedSave().Inventory or {}).Misc or {}) do
                                ...
                                task.spawn(function()
                                    ...
                                    if zoneFolder then
                                        ...
                                        if hrp then
                                            ...
                                            local newInv = (GetCachedSave().Inventory or {}).Misc or {}
                ...
                if quest.name == "COLLECT_POTION" or quest.name == "COLLECT_ENCHANT" then
                    if not actionTakenThisLoop then
                        ...
                        if os.clock() - (vm:Get("ActionTime_" .. quest.goalId) or 0) > 4 then 
                            ...
                            setthreadidentity(4)
                            for uid, dat in pairs((GetCachedSave().Inventory or {})[isPotion and "Potion" or "Enchant"] or {}) do
