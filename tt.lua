...
for questName, defaultPrio in pairs({
    ...
    BEST_MINI_CHEST = 7, HATCH_RARE_PET = 1, CURRENCY = 7,	
                        ...
                        if os.clock() - lastTime > 2.5 then 
                            ...
                            task.spawn(function()
                                ...
                                if quest.name == "HATCH_RARE_PET" then
                                    -- Kiểm tra phải đang ở đúng World 4 (Void World) mới thực thi
                                    if GetCurrentWorldNumber() == 4 then
                                        local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
                                        if hrp then
                                    ...
                                    -- Nếu KHÔNG ở World 4, script sẽ lọt xuống dưới và chạy HatchBestEgg() làm phương án dự phòng
                                end
